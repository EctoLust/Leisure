...
function Combat:GetAttacked(attacker, damage, weapon, stimuli)
    ...
    if self.inst.components.health ~= nil and damage ~= nil and damageredirecttarget == nil then
        ...
        if damage > 0 and not self.inst.components.health:IsInvincible() then
			...
			if self.inst:HasTag("CantAttackAtFuck") then
				return false
			end	
			
			if self.inst.components.inventory and attacker and attacker:HasTag("MONSTER_PERVERT") then
			    local body_item = self.inst.components.inventory:GetEquippedItem(EQUIPSLOTS.BODY)
				if body_item and body_item:HasTag("plug") then					
					if self.inst.plugprotection == nil or self.inst.plugprotection == 0 then
					    self.inst.SoundEmitter:PlaySound("dontstarve/wilson/use_break")
				        self.inst.plugprotection = 7
						if body_item.components.finiteuses ~= nil then
						    body_item.components.finiteuses:Use()
						else
						    body_item:Remove();
						end
					    
					    self.inst.components.talker:Say("My butt ins't for you!")						
					else
					    self.inst.SoundEmitter:PlaySound("dontstarve/wilson/use_break", nil, 0.3)
					end
		    ...
		    if attacker and attacker:HasTag("MONSTER_PERVERT") and self.inst:HasTag("player") and self.inst.components.health
		    --and self.inst.components.health and damage and self.inst.components.health.currenthealth-damage <= 0 then
			and self.inst.components.health and damage and not self.inst:HasTag("CantAttackAtFuck") and self.inst.waitabit == nil and TUNING.MONSTERS_RAPE == 1 then
		        --print("Health of player "..self.inst.components.health.currenthealth)
			    --print("Damage of mob "..damage)
			    --print("Health - Damgae "..self.inst.components.health.currenthealth-damage)
				--self.inst.components.health.currenthealth = 2
				self.inst.components.health:DoDelta(-1)		        
				self.inst.components.health:SetInvincible(true)
				if attacker.components.sex then
			        attacker.components.sex:MonsterFuckPlayer(attacker, self.inst, damage)
				end
				
				if attacker.prefab == "eyeplant" then
				    attacker.components.combat.onhitotherfn(attacker, self.inst, damage, stimuli)
				end
				
				
				return false			
			elseif not self.inst:HasTag("CantAttackAtFuck") and (self.inst.sg == nil or not self.inst.sg:HasStateTag("dead")) then
